...
describe Booqable do
  describe ".parse_resource" do
    ...
    it "parses JSON:API payload into a Sawyer::Resource" do
      result = described_class.parse_resource(payload)
      expect(result).to be_a(Sawyer::Resource)
      expect(result.name).to eq("John")
    end
  end

  describe ".deserialize_resource" do
    let(:payload) do
      {
        "data" => {
          "id" => "123",
    ...
    it "is an alias for parse_resource" do
      result = described_class.deserialize_resource(payload)
      expect(result).to be_a(Sawyer::Resource)
      expect(result.name).to eq("John")
    end
  end
...
describe Booqable::Client do
  describe "#parse_resource" do
    ...
    it "parses JSON:API payload into a Sawyer::Resource" do
      result = client.parse_resource(payload)
      expect(result).to be_a(Sawyer::Resource)
      expect(result.status).to eq("reserved")
    end
  end
  ...
  describe "#deserialize_resource" do
    ...
    it "is an alias for parse_resource" do
      result = client.deserialize_resource(payload)
      expect(result).to be_a(Sawyer::Resource)
      expect(result.status).to eq("reserved")
    end
  end
