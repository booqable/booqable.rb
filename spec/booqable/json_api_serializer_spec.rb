...
describe Booqable::JsonApiSerializer do
  describe ".any_json" do
    it "raises error when no JSON gem is available" do
      # Mock all JSON gems to be unavailable
      allow(Booqable::JsonApiSerializer).to receive(:yajl).and_return(nil)
      allow(Booqable::JsonApiSerializer).to receive(:multi_json).and_return(nil)
      allow(Booqable::JsonApiSerializer).to receive(:json).and_return(nil)

      expect { Booqable::JsonApiSerializer.any_json }.to raise_error(RuntimeError, "Sawyer requires a JSON gem: yajl, multi_json, or json")
    end
  end

  describe ".yajl" do
    it "returns yajl serializer when available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("yajl")
      ...
      result = Booqable::JsonApiSerializer.yajl
      expect(result).to be_a(Booqable::JsonApiSerializer)
    end

    it "returns nil when yajl is not available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("yajl").and_raise(LoadError)
  ...
  describe ".json" do
    it "returns json serializer when available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("json")
      ...
      result = Booqable::JsonApiSerializer.json
      expect(result).to be_a(Booqable::JsonApiSerializer)
    end

    it "returns nil when json is not available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("json").and_raise(LoadError)
  ...
  describe ".multi_json" do
    it "returns multi_json serializer when available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("multi_json")
      ...
      result = Booqable::JsonApiSerializer.multi_json
      expect(result).to be_a(Booqable::JsonApiSerializer)
    end
  end
  ...
  describe ".message_pack" do
    it "returns message_pack serializer when available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("msgpack")
      ...
      result = Booqable::JsonApiSerializer.message_pack
      expect(result).to be_a(Booqable::JsonApiSerializer)
    end

    it "returns nil when msgpack is not available" do
      allow(Booqable::JsonApiSerializer).to receive(:require).with("msgpack").and_raise(LoadError)
  ...
  describe "instance methods" do
    let(:serializer) { Booqable::JsonApiSerializer.new(JSON) }
